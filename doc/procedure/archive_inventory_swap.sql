...
BEGIN
	...
	DECLARE TransactionCursor CURSOR
		...
		FOR SELECT
			...
			i.last_modified_user,i.item_cost_amt,i.item_desc,i.item_desc2,i.cat_id,i.alert_msg,i.swap_from,i.swap_to,i.swap_source,i.retry_count
		FROM
	...
	FETCH NEXT FROM TransactionCursor INTO @swap_id,@item_nbr,@club_nbr,@item_qty,@swap_type,@swap_state,@error_msg,@created_ts,@created_user,@last_modified_ts,
		@last_modified_user,@item_cost_amt,@item_desc,@item_desc2,@cat_id,@alert_msg,@swap_from,@swap_to,@swap_source,@retry_count;
		...
		BEGIN TRY
			...
			WHILE (@BatchTranSize < @BATCH_SIZE) BEGIN
				IF (@@FETCH_STATUS = 0) BEGIN
					...
					insert into claims_archive.inventory_swap1 (swap_id,item_nbr,club_nbr,item_qty,swap_type,swap_state,error_msg,created_ts,created_user,last_modified_ts,
						last_modified_user,item_cost_amt,item_desc,item_desc2,cat_id,alert_msg,swap_from,swap_to,swap_source,retry_count,archived_ts)
					values  (@swap_id,@item_nbr,@club_nbr,@item_qty,@swap_type,@swap_state,@error_msg,@created_ts,@created_user,@last_modified_ts,@last_modified_user,
						@item_cost_amt,@item_desc,@item_desc2,@cat_id,@alert_msg,@swap_from,@swap_to,@swap_source,@retry_count,SYSDATETIME());
				END
				ELSE BREAK;
				IF (@@ERROR <> 0) BEGIN
